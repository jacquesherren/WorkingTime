...
public class TaskAsyncTaskRemove extends AsyncTask<Void, Void, List<Task>> {
    private static TaskApi myApiTaskService = null;
    private static final String TAG = TaskAsyncTaskRemove.class.getName();
    private Task task;

    public TaskAsyncTaskRemove()
    {
        this.task=task;
    ...
    protected List<Task> doInBackground(Void... params) {
        ...
        try{
            ...
            // For instance insert
            if(myApiTaskService.list().execute().getItems() != null)
            {
                List<Task> tasks = new ArrayList<Task>();
                tasks = myApiTaskService.list().execute().getItems();
                long listSize = tasks.size();
                Log.i(TAG, "size : " + listSize );
                for (Task t : tasks)
                {
                    myApiTaskService.remove(t.getId()).execute();
