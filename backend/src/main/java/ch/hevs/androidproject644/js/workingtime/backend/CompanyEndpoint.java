...
public class CompanyEndpoint {
    ...
    public Company get(@Named("_id") long _id) throws NotFoundException {
        logger.info("Getting Company with ID: " + _id);
        Company company = ofy().load().type(Company.class).id(_id).now();
        if (company == null) {
            throw new NotFoundException("Could not find Company with ID: " + _id);
        }
        return company;
    }
    ...
    public Company insert(Company company) {
        ...
        // You should validate that company._id has not been set. If the ID type is not supported by the
        // Objectify ID generator, e.g. long or String, then you should generate the unique ID yourself prior to saving.
        //
        // If your client provides the ID then you should probably use PUT instead.
        ofy().save().entity(company).now();
        logger.info("Created Company with ID: " + company.get_id());
    ...
    public Company update(@Named("_id") long _id, Company company) throws NotFoundException {
    ...
    public void remove(@Named("_id") long _id) throws NotFoundException {
        checkExists(_id);
        ofy().delete().type(Company.class).id(_id).now();
        logger.info("Deleted Company with ID: " + _id);
    }
    ...
    private void checkExists(long _id) throws NotFoundException {
        try {
            ofy().load().type(Company.class).id(_id).safe();
