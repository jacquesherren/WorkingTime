...
public class WorkerEndpoint {
    ...
    public Worker get(@Named("_id") long _id) throws NotFoundException {
        logger.info("Getting Worker with ID: " + _id);
        Worker worker = ofy().load().type(Worker.class).id(_id).now();
        if (worker == null) {
            throw new NotFoundException("Could not find Worker with ID: " + _id);
        }
        return worker;
    }
    ...
    public Worker insert(Worker worker) {
        ...
        // You should validate that worker._id has not been set. If the ID type is not supported by the
        // Objectify ID generator, e.g. long or String, then you should generate the unique ID yourself prior to saving.
        //
        // If your client provides the ID then you should probably use PUT instead.
        ofy().save().entity(worker).now();
        logger.info("Created Worker with ID: " + worker.get_id());
    ...
    public Worker update(@Named("_id") long _id, Worker worker) throws NotFoundException {
    ...
    public void remove(@Named("_id") long _id) throws NotFoundException {
        checkExists(_id);
        ofy().delete().type(Worker.class).id(_id).now();
        logger.info("Deleted Worker with ID: " + _id);
    }
    ...
    private void checkExists(long _id) throws NotFoundException {
        try {
            ofy().load().type(Worker.class).id(_id).safe();
